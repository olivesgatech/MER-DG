...
import wandb
from losses import  mer_dg_loss , mer_dg_loss
...
def train_one_step(clip, labels, flow, spectrogram):
    ...
    if args.use_video:
        ...
        v_s_emb_proj = v_proj_s(v_emd[:, v_dim:])
    if args.use_audio:
        a_emd_proj = a_proj(audio_emd[:, :a_dim])
        a_s_emb_proj = a_proj_s(audio_emd[:, a_dim:])
    if args.use_flow:
        f_emd_proj = f_proj(f_emd[:, :f_dim])
        f_s_emb_proj = f_proj_s(f_emd[:, f_dim:])
    ...
    if args.use_video and args.use_flow and args.use_audio:
        ...
        emd_proj = torch.stack([f_emd_proj, a_emd_proj], dim=1)     # 16, 2 ,128
    ...
    # Variance-Covariance Loss: Apply to all modality pairs
    loss_mer = 0.0
    
    # Count how many modalities we have
    modalities_used = []
    if args.use_video:
        modalities_used.append(('video', v_emd_proj, v_s_emb_proj))
    if args.use_audio:
        modalities_used.append(('audio', a_emd_proj, a_s_emb_proj))
    if args.use_flow:
        modalities_used.append(('flow', f_emd_proj, f_s_emb_proj))
    
                
    if modalities_used:
        for _, z_c , z_s in modalities_used:
            loss_mer += mer_dg_loss(z_c, alpha_marg=args.alpha_marg, alpha_spec=args.alpha_spec) + mer_dg_loss(z_s, alpha_marg=args.alpha_marg, alpha_spec=args.alpha_spec)
...
if __name__ == '__main__':
    ...
    if args.use_video:
        ...
        v_proj_s = ProjectHead(input_dim=1152, hidden_dim=args.hidden_dim, out_dim=args.out_dim).cuda()
    ...
    if args.use_flow:
        ...
        f_proj_s = ProjectHead(input_dim=1024, hidden_dim=args.hidden_dim, out_dim=args.out_dim).cuda()
    ...
    if args.use_audio:
        ...
        a_proj_s = ProjectHead(input_dim=256, hidden_dim=args.hidden_dim, out_dim=args.out_dim).cuda()
    ...
    base_path_model = os.path.join("models", run_name)
    if not os.path.exists(base_path_model):
        os.makedirs(base_path_model)
    
    # Create checkpoint folder with wandb run name only if checkpoint saving is enabled
    if args.save_checkpoint or args.save_best:
        checkpoint_folder = os.path.join("checkpoints", run_name)
        if not os.path.exists(checkpoint_folder):
            os.makedirs(checkpoint_folder)
    else:
        checkpoint_folder = base_path_model
    ...
    if args.use_video:
        ...
        model.module.backbone.slow_path.layer4.parameters()) + list(model.module.cls_head.parameters()) + list(v_proj.parameters()) + list(v_proj_s.parameters())
    if args.use_flow:
        params = params + list(model_flow.module.backbone.layer4.parameters()) +list(model_flow.module.cls_head.parameters()) + list(f_proj.parameters()) + list(f_proj_s.parameters())
    if args.use_audio:
        params = params + list(audio_cls_model.parameters()) + list(a_proj.parameters()) + list(a_proj_s.parameters())
    ...
    if args.resumef:
        resume_file = checkpoint_folder + '/' + log_name + '.pt'
        ...
        if args.use_video:
            ...
            if 'v_proj_s_state_dict' in checkpoint:
                v_proj_s.load_state_dict(checkpoint['v_proj_s_state_dict'])
        if args.use_flow:
            model_flow.load_state_dict(checkpoint['model_flow_state_dict'])
            f_proj.load_state_dict(checkpoint['f_proj_state_dict'])
            if 'f_proj_s_state_dict' in checkpoint:
                f_proj_s.load_state_dict(checkpoint['f_proj_s_state_dict'])
        if args.use_audio:
            ...
            if 'a_proj_s_state_dict' in checkpoint:
                a_proj_s.load_state_dict(checkpoint['a_proj_s_state_dict'])
                    ...
                    if split == 'test':
                        ...
                        if currenttestAcc >= BestTestAcc:
                            ...
                            if args.save_best:
                                ...
                                torch.save(save, os.path.join(checkpoint_folder, log_name + '_best_%s.pt'%(str(epoch_i))))
                        ...
                        if args.save_checkpoint:
                            ...
                            torch.save(save, os.path.join(checkpoint_folder, log_name + '.pt'))
